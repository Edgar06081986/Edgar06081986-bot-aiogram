...
def get_or_create_logger(source):
    ...
    handler = TimedRotatingFileHandler(
        log_file_path,
        when="midnight",
        interval=Config.LOG_ROTATE_DAYS,
        backupCount=5,
        encoding='utf-8'
    )
...
def log_event(source: str, level: str, message: str) -> None:
    ...
    :param source: источник события. Только латинские символы
    :param level: Уровень события. LogTypesEnum содержит возможные типы
    :param message: Сообщение, подлежащее записи в лог
    """
...
def log_user_event(level: str, user_id: int, message: str) -> None:
    """
    Логирование событий по пользователю
    """
...
def log_system_event(level: str, message: str) -> None:
    """
    Логирование событий с источником "Система"
    """
